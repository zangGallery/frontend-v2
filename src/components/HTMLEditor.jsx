...
const AceEditor = lazy(() => import("react-ace").then(async (mod) => {
    // Load ace modes, themes, and extensions
    await import("ace-builds/src-noconflict/mode-html");
    await import("ace-builds/src-noconflict/theme-monokai");
    await import("ace-builds/src-noconflict/ext-language_tools");

    // Disable workers to avoid loading issues
...
const Split = lazy(() => import("@geoffcox/react-splitter").then(mod => ({ default: mod.Split })));

function EditorContent({ value, setValue, setPreviewOpen }) {
    return (
        <div
...
export default function HTMLEditor({ value, setValue }) {
    ...
    return (
        <Fragment>
            <Suspense
                fallback={
                    <div className="rounded-lg border border-ink-700 bg-ink-900 flex items-center justify-center" style={{ height: "500px" }}>
                        <div className="text-ink-400">Loading editor...</div>
                    </div>
                }
            >
                <EditorContent value={value} setValue={setValue} setPreviewOpen={setPreviewOpen} />
            </Suspense>
